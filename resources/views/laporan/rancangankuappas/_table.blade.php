...
<body>
  ...
  <div class="">
    <input type="button" onclick="tableToExcel('testTable', 'Sheet1')" value="Export to Excel">
  </div>
...
<table width="1400" height="550" border="0" cellspacing="0" cellpadding="4" align="center" class="wrapper" id="testTable">
    ...
    <tr>
        <td>
            <h2>HASIL RANCANGAN KUA PPAS</h2>
            <h2>
        ...
        <td>
            <table width="100%" border="0" cellspacing="0" cellpadding="3" class="data_table">
                ...
                <tr style="border-bottom:#999 solid 2px;">
                    ...
                    <th rowspan="3" width="100">PD Pelaksana</th>
                    <!-- <th rowspan="3" width="100">Pengusul</th> -->
                ...
                @foreach (json_decode($items) as $idx => $anggaran)
                    <tr>
                        ...
                        <td style="text-align:right;">{{ $anggaran->pagu }}</td>
                        <td style="text-align:left;">{{ $anggaran->sumber_anggaran->nama }}</td>
                        <td style="text-align:left;">{{ $anggaran->lokasi }}</td>
                        <td style="text-align:left;">{{ $anggaran->opd_pelaksana->nama }}</td>
                        <!-- <td style="text-align:left;"><?php print_r($anggaran->first_create);?></td> -->
                        <td style="text-align:left;">{{ get_kecamatan_from_location($anggaran->lokasi) }}</td>
                        <td style="text-align:left;">{{ get_desa_from_location($anggaran->lokasi) }}</td>
                    </tr>
                @endforeach
