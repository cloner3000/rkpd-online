            ...
            <div class="mr-auto">
                ...
                <ul class="m-subheader__breadcrumbs m-nav m-nav--inline">
                    ...
                    <li class="m-nav__item">
                        <a href="{{ route('musrenbang-kelurahan.index') }}" class="m-nav__link">
                            <span class="m-nav__link-text">{{ $title ?? 'Laporan' }} Musrenbang Desa/Kelurahan</span>
                        </a>
                    </li>
            </div>
        </div>
    ...
    <div class="m-content">
        <div class="m-portlet m-portlet--mobile">
            <div class="m-portlet__head">
                <div class="m-portlet__head-caption">
                    <div class="m-portlet__head-title">
                        <h3 class="m-portlet__head-text">{{ $title ?? 'Laporan' }} Musrenbang Desa/Kelurahan</h3>
                    </div>
                </div>
            </div>
            <div class="m-portlet__body">
                ...
                <form action="{{ route('laporan.desa.preview') }}" method="post">
                    {{ csrf_field() }}
                        ...
                        <div class="col-md-2">
                            <div class="form-group m-form__group {{ $errors->has('tahun') ? 'has-danger' : ''}}">
                                ...
                                <select class="form-control m-select2" id="m_select2_1" disabled>
                                    <option value="{{ $item->tahun ?? (Carbon\Carbon::now()->year + 1)  }}">{{ $item->tahun ?? (Carbon\Carbon::now()->year + 2)  }}</option>
                                </select>
                    ...
                    <div class="form-group">
                        <div class="col-12">
                            <div class="form-group m-form__group">
                                ...
                                <select class="form-control m-select2" name="kecamatan" disabled>
                                    <option disabled selected>-- Silahkan Pilih --</option>
                                    <option selected
                                            value="{{ $village->district->id }}">{{ $village->district->name }}</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-12">
                    ...
                    <input type="hidden" name="district_id" value="{{ $village->district->id }}">
                    @endrole
