            ...
            <div class="m-portlet__head">
                <div class="m-portlet__head-caption">
                    <div class="m-portlet__head-title">
                        <h3 class="m-portlet__head-text">Hasil Pokok Pikiran Dewan</h3>
                    </div>
                </div>
            </div>
                            ...
                            <div class="form-group m-form__group row align-items-center">
                                <div class="col-md-7">
                                    @include('global.table_search', [
                                       'action' => route('musrenbang-dewan.index', ['url'=>'items']),
                                       'search' => $search ?? ""
                                   ])
                                </div>
                            </div>
                        </div>
                        @if ($canEntry && $user->hasRole(\App\Enum\Roles::DPRD))
                        <div class="col-xl-4 order-1 order-xl-2 m--align-right">
                            <a href="{{ route('musrenbang-dewan.create') }}"
                               class="btn btn-accent m-btn m-btn--custom m-btn--icon m-btn--air m-btn--pill">
                                <span><i class="la la-plus"></i>
                                    <span>Tambah Pokok Pikiran Dewan</span>
                                </span>
                            </a>
                        ...
                        <tr>
                            ...
                            <td>
                                @include('global.table_action', [
                                    'action' => route('musrenbang-dewan.destroy', ['id' => $item->id]),
                                    'url'    => route('musrenbang-dewan.edit', ['id' => $item->id]),
                                    'id'     => $item->id,
                                    'show'   => route('musrenbang-dewan.show', $item->id),
                                    'transfer' => route('musrenbang-dewan.transfer.view', ['id' => $item->id]),
                                    'isViewTransfer' => true,
                                ])
                            </td>
                        </tr>
